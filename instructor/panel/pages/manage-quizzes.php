                    ...
                    <div class="table-responsive" style="padding-left: 15px; padding-right: 15px; padding-bottom: 15px; padding-top: 15px;">
                        <table class="data-table align-middle mb-0 table table-borderless table-hover" id="tableList">
                            ...
                            <tbody>
                              <?php
                              if (!empty($course_id)) {
                                $selQuiz   = $conn->query("SELECT * FROM quiz_tbl WHERE cou_id ='$course_id'");
                              }else{
                                $selQuiz = $conn->query("SELECT * FROM quiz_tbl WHERE cou_id IN (SELECT cou_id from course_tbl Where cou_instructor = $id) ORDER BY quiz_id DESC ");
                              } 
