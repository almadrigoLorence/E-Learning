                ...
                <div class="page-title-wrapper" >
                      <div class="page-title-heading">
                          <div class="page-title-icon"><img src="assets/images/exam1.png" height="40" width="40"></div>
                            <div>Activities <span><?php if (!empty($course_name)) {echo $course_name;}?></span>
                    ...
                    <div class="table-responsive" style="padding-left: 15px; padding-right: 15px; padding-bottom: 15px; padding-top: 15px;">
                        <table class="data-table align-middle mb-0 table table-borderless table-hover" id="tableList">
                            ...
                            <tbody>
                              <?php 
                              if (!empty($course_id)) {
                                 $selAct = $conn->query("SELECT * FROM activities_tbl WHERE cou_id ='$course_id'");
                              }else{
                                $selAct = $conn->query("SELECT * FROM activities_tbl WHERE cou_id IN (SELECT cou_id from course_tbl Where cou_instructor = $id) ORDER BY act_id DESC ");
                              }
                                if($selAct->rowCount() > 0)
                                {
                                    while ($selActRow = $selAct->fetch(PDO::FETCH_ASSOC)) { ?>
                                        <tr>
                                            <td class="text-left" width="20%"><?php echo $selActRow['act_title']; ?></td>
                                            <td class="text-center" width="20%"><i class="pe-7s-study"></i>
                                                <?php 
