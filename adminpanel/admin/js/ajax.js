  ...
  $.post("query/addQuizExe.php", $(this).serialize() , function(data){
    ...
    if(data.res == "noDisplayLimit")
   {
    ...
    else if(data.res == "success")
    {
      Swal.fire(
        'Success',
        data.quizTitle.toUpperCase() + '<br>Successfully Added',
        'success'
      )
          $('#addQuizFrm')[0].reset();
          $('#course_name').val("");
...
// edit Topic 
$(document).on("submit","#editTopicFrm" , function(){
  $.post("query/editTopicExe.php", $(this).serialize() , function(data){
    if(data.res == "success")
    {
      Swal.fire(
        'Success',
          ...
          $.ajax({
           ...
           beforeSend: function() {
              ...
              if (n == cn) {
                ...
                alert("Incorrect Old password or Password doenst matches")
                return false;
              }
           },
           success: function(data)
